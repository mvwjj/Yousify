    ...
    private class TokenAuthenticator(private val context: Context) : Authenticator {
        override fun authenticate(route: Route?, response: okhttp3.Response): Request? {
            ...
            if (currentRefreshToken == null) {
                ...
                // Если нет refresh token, инициируем принудительный выход, т.к. восстановить сессию невозможно
                AuthEvents.triggerForceLogoutNonSuspend() // Используем non-suspend версию
                return null
            }
            ...
            synchronized(this) {
                ...
                return if (refreshedSuccessfully) {
                    ...
                    // triggerForceLogoutNonSuspend() здесь уже был вызван внутри refreshTokenSync при invalid_grant
                    null
                }
            }
        }
        ...
        private fun refreshTokenSync(refreshToken: String): Boolean {
            ...
            return try {
                ...
                if (responseCode == 200) {
                    ...
                    Logger.i("TokenAuthenticator: Token refreshed and saved. New access token ends with ${newAccessToken.takeLast(5)}")
                    true
                } else {
                    ...
                    if (responseCode == 400 && responseBody.contains("invalid_grant", ignoreCase = true)) {
                        ...
                        AuthEvents.triggerForceLogoutNonSuspend() // ИЗМЕНЕНО: Используем non-suspend
                    }
                    false
                }
            } catch (e: IOException) {
